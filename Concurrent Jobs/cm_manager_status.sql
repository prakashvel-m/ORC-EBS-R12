...
SELECT 
    ...
    (a.MAX_PROCESSES - SUM(DECODE(b.PHASE_CODE, 'R', DECODE(b.STATUS_CODE, 'R', 1, 0), 0))) "FREE_SLOTS",
    CASE 
        WHEN a.MAX_PROCESSES > 0 THEN
            ROUND((((a.MAX_PROCESSES - SUM(DECODE(b.PHASE_CODE, 'R', DECODE(b.STATUS_CODE, 'R', 1, 0), 0))) / a.MAX_PROCESSES) * 100), 0)
        ELSE
            0
    END "FREE_%",
    SUM(DECODE(b.PHASE_CODE, 'R', DECODE(b.STATUS_CODE, 'R', 1, 0), 0)) "RUNNING_NORMAL",
...
JOIN 
    APPS.FND_CONCURRENT_WORKER_REQUESTS b ON a.concurrent_queue_id = b.concurrent_queue_id
WHERE 
